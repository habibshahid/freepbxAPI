...
class FreePbxController extends Controller
{
    ...
    private function getDestinationIdByType($type, $name){
        switch($type){
            ...
            case 'trunks':
                ...
                break;
            case 'playback':
                $sql = "SELECT id as id FROM recordings where displayname = '". $name ."';";
                $stmt = $this->c->db->query($sql);
                $result = $stmt->fetch();
                return $result['id'];
                break;
    ...
    private function checkDuplicateOutboundTrunk($routeId, $trunkId){
        $sql = "SELECT * FROM outbound_route_trunks where trunk_id = '".$trunkId."' and route_id = '".$routeId."'";
        $stmt = $this->c->db->query($sql);
    ...
    private function checkDuplicateInboundRouteDIDExtension($did, $extension){
        $sql = "SELECT * FROM incoming where extension = '" . $did . "' and destination = 'from-did,". $extension .",1';";
        $stmt = $this->c->db->query($sql);
        $duplicate = $stmt->fetchAll();
    ...
    private function checkDuplicateOutboundDialPattern($dialPattern, $routeId){
    ...
    private function checkDuplicateInboundDialPattern($dialPattern, $trunkId){
        $sql = "SELECT * FROM trunk_dialpatterns where trunkid = '". $trunkId ."' and match_pattern_prefix = '". $dialPattern['match_pattern_prefix'] ."' and match_pattern_pass = '". $dialPattern['match_pattern_pass'] ."' and prepend_digits = '". $dialPattern['prepend_digits'] ."' and seq = '". $dialPattern['seq'] ."';";
        $stmt = $this->c->db->query($sql);
        $result = $stmt->fetchAll();
    ...
    private function getExtensionDetails($extension){
        $sql = "SELECT * FROM devices where id = '" . $extension . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function getExtensionUserDetails($extension){
        $sql = "SELECT * FROM userman_users where username = '" . $extension . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function getTrunkDetails($name){
        $sql = "SELECT * FROM trunks where name = '" . $name . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function getPeerContextDetails($id){
        $sql = "SELECT * FROM sip where id = 'tr-peer-" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function getUserContextDetails($id){
        $sql = "SELECT * FROM sip where id = 'tr-user-" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function deleteTrunkPeerContext($id){
        $sql = "SELECT * FROM sip where id = 'tr-peer-" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->execute();
        return $extension;
    }
    private function deleteTrunkUserContext($id){
        $sql = "SELECT * FROM sip where id = 'tr-user-" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->execute();
        return $extension;
    }
    private function deleteTrunkRegisterString($id){
        $sql = "SELECT * FROM sip where id = 'tr-reg-" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->execute();
        return $extension;
    }
    private function deleteTrunkPJSIP($id){
        $sql = "SELECT * FROM pjsip where id = '" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->execute();
        return $extension;
    }
    private function getTrunkPJSIP($id){
        $sql = "SELECT * FROM pjsip where id = '" . $id . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function getInboundRouteDetails($name, $did){
        $sql = "SELECT * FROM incoming where description = '" . $name . "' and extension = '". $did ."';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    }
    private function getOutboundRouteDetails($name){
        $sql = "SELECT * FROM outbound_routes where name = '" . $name . "';";
        $stmt = $this->c->db->query($sql);
        $extension = $stmt->fetch();
        return $extension;
    ...
    public function createSIPExtension($request, $response){
        ...
        $ampuser = array(
            ...
            array(
                "key"   => "/AMPUSER/". $body['extension'] ."/pinless",
            ...
            array(
                "key" => "/DEVICE/". $body['extension'] ."/dial",
                "value" => $dial
            ),
            array(
        ...
        if (isset($body['incomingdid']) && $body['incomingdid'] != '') {
            $destinationFormat = 'from-did,' . $body['extension'] . ',1';
            $duplicate['incoming'] = $this->checkDuplicateInboundRouteDIDExtension($body['incomingdid'], $destinationFormat);
    ...
    public function updateSIPExtension($request, $response, $args){
        global $chan_drivers, $followMeStrategies, $destinationTypes;
        ...
        $extensionDetails = $this->getExtensionDetails($body['extension']);
        $userDetails = $this->getExtensionUserDetails($body['extension']);
        ...
        $sql = "update devices set tech = :tech, dial = :dial, devicetype = :devicetype, user = :user, description = :description, emergency_cid = :emergency_cid, hint_override = :hint_override where id = :extension";
        $stmt = $this->c->db->prepare($sql);
        foreach ($devices as $key => &$val) {
            $stmt->bindParam($key, $val);
        }
        ...
        foreach ($ampuser as $setting) {
            try {
                $astDBSQL = "update astdb set value = :value where key = :key";
                $stmt = $this->c->sqlite->prepare($astDBSQL);
                foreach ($setting as $key => &$val) {
                    $stmt->bindParam($key, $val);
                }
        ...
        if($body['followme']['enabled'] == "1"){
            ...
            $sql = "update findmefollow set strategy = :strategy, grptime = :grptime, grppre = :grppre, grplist = :grplist, annmsg_id = :annmsg_id, postdest = :postdest, dring = :dring, rvolume = :rvolume, remotealert_id = :remotealert_id, needsconf = :needsconf, toolate_id = :toolate_id, pre_ring = :pre_ring, ringing = :ringing, calendar_enable = :calendar_enable, calendar_id = :calendar_id, calendar_group_id = :calendar_group_id, calendar_match = :calendar_match where grpnum = :extension";
            $stmt = $this->c->db->prepare($sql);
            foreach ($findmefollow as $key => &$val) {
                $stmt->bindParam($key, $val);
            }
        ...
        if (isset($body['incomingdid']) && $body['incomingdid'] != '') {
            $duplicate['incoming'] = $this->checkDuplicateInboundRouteDIDExtension($body['incomingdid'], $body['extension']);
            $incoming = array(
                'cidnum'            =>  (isset($body['cidnum'])) ? $body['cidnum'] : '',
                'extension'         =>  $body['incomingdid'],
            ...
            if (!$duplicate['incoming']) {
                ...
                $sql = "update incoming set cidnum = :cidnum, extension = :extension, destination = :destination, privacyman = :privacyman, alertinfo = :alertinfo, ringing = :ringing, fanswer = :fanswer, mohclass = :mohclass, description = :description, grppre = :grppre, delay_answer = :delay_answer, pricid = :pricid, pmmaxretries = :pmmaxretries, pmminlength = :pmminlength, reversal = :reversal, rvolume = :rvolume, indication_zone = :indication_zone where extension = :extension and destination = :destination";
                $stmt = $this->c->db->prepare($sql);
                foreach ($incoming as $key => &$val) {
                    $stmt->bindParam($key, $val);
                }
        ...
        if($extensionDetails['tech'] != $tech){
            $delExten['extension'] = $body['extension'];
            $sql = "delete from sip where id = :extension";
            $stmt = $this->c->db->prepare($sql);
            foreach ($delExten as $key => &$val) {
                $stmt->bindParam($key, $val);
            }
            $sipResult = $stmt->execute();
            $result['deleteSIP'] = $sipResult;
            ...
            foreach ($settings as $setting) {
                $sql = "update sip set keyword = :keyword, data = :data, flags = :flags where id = :id and keyword = :keyword";
                $stmt = $this->c->db->prepare($sql);
                foreach ($setting as $key => &$val) {
                    $stmt->bindParam($key, $val);
                }
        ...
        if ($userDetails) {
            ...
            $sql = "update userman_users set auth = :auth, authid = :authid, username = :username, description = :description, password = :password, default_extension = :default_extension, primary_group = :primary_group, permissions = :permissions, fname = :fname, lname = :lname, displayname = :displayname, title = :title, company = :company, department = :department, language = :language, timezone = :timezone, dateformat = :dateformat, timeformat = :timeformat, datetimeformat = :datetimeformat, email = :email, cell = :cell, work = :work, home = :home, fax = :fax where id = :id";
            $stmt = $this->c->db->prepare($sql);
            foreach ($userman_users as $key => &$val) {
                $stmt->bindParam($key, $val);
            }
    ...
    public function deleteSIPExtension($request, $response, $args){
        if (!isset($args['extension']) || $args['extension'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Extension not defined.'
        ...
        $sql = "delete FROM devices where id = '". $args['extension'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['devices'] = $stmt->execute();

        $sql = "delete FROM sip where id = '". $args['extension'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['sip'] = $stmt->execute();

        $sql = "delete FROM astdb where key like '%/". $args['extension'] ."%'";
        $stmt = $this->c->sqlite->query($sql);
        $result['astdb'] = $stmt->execute();

        $sql = "delete FROM findmefollow where grpnum = '". $args['extension'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['findmefollow'] = $stmt->execute();

        $sql = "delete FROM findmefollow where grpnum = '". $args['extension'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['findmefollow'] = $stmt->execute();

        $sql = "delete FROM userman_users where username = '". $args['extension'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['userman_users'] = $stmt->execute();

        $sql = "delete FROM users where extension = '". $args['extension'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['users'] = $stmt->execute();
    ...
    public function updateSIPTrunk($request, $response, $args){
        ...
        if (!isset($args['trunkName']) || $args['trunkName'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Trunk Name not defined.'
            ));
        ...
        $trunkDetails = $this->getTrunkDetails($args['trunkName']);
        ...
        $trunkId = $trunkDetails['trunkid'];

        if($body['trunk']['tech'] == 'sip'){
            $peerContextDetails = $this->getPeerContextDetails($trunkId);
            $userContextDetails = $this->getUserContextDetails($trunkId);
        }
        ...
        $trunkSQL = "update trunks set tech = :tech, outcid = :outcid, keepcid = :keepcid, maxchans = :maxchans, failscript = :failscript, dialoutprefix = :dialoutprefix, provider = :provider, disabled = :disabled, `continue` = :continue where trunkid = :trunkid";
        $stmt = $this->c->db->prepare($trunkSQL);
        foreach ($trunk as $key => &$val) {
            $stmt->bindParam($key, $val);
        }
        ...
        if (isset($body['dialpatterns']) && count($body['dialpatterns']) > 0) {
            foreach ($body['dialpatterns'] as $dialPattern) {
                ...
                $duplicate = $this->checkDuplicateInboundDialPattern($dialPatterns, $trunkId);
                if($duplicate){
                    $dialPatternSQL = "update trunk_dialpatterns set match_pattern_prefix = :match_pattern_prefix, match_pattern_pass = :match_pattern_pass, prepend_digits = :prepend_digits, seq = :seq where trunkid = :trunkid and match_pattern_prefix = :match_pattern_prefix and match_pattern_pass = :match_pattern_pass and prepend_digits = :prepend_digits and seq = :seq";
                    $stmt = $this->c->db->prepare($dialPatternSQL);
                    foreach ($dialPatterns as $key => &$val) {
                        $stmt->bindParam($key, $val);
                    }
                    $dialPatternResult = $stmt->execute();
                    $result['dialplan_patterns'] = $dialPatternResult;
        ...
        if ($body['trunk']['tech'] == 'sip') {
            if($body['trunk']['tech'] != $trunkDetails['tech']){
                $this->deleteTrunkPJSIP($trunkId);
            }

            if (isset($body['peercontext']) && count($body['peercontext']) > 0) {
                $this->deleteTrunkPeerContext($trunkId);
                $peerContextCount = 1;
                foreach ($body['peercontext'] as $key => &$val) {
                    $sip[] = array(
                        'id' => 'tr-peer-' . $trunkId,
            ...
            if (isset($body['usercontext']) && count($body['usercontext']) > 0) {
                $this->deleteTrunkUserContext($trunkId);
                $userContextCount = 1;
                foreach ($body['usercontext'] as $key => &$val) {
                    $sip[] = array(
                        'id' => 'tr-user-' . $trunkId,
            ...
            if (isset($body['registerstring']) || $body['registerstring'] != '') {
                $this->deleteTrunkRegisterString($trunkId);
                $sip[] = array(
                    'id' => 'tr-reg-' . $trunkId,
                    'keyword' => 'register',
                    'data' => $body['registerstring'],
            ...
            $PJSIP = $this->getTrunkPJSIP($trunkId);
            if($PJSIP){
                foreach ($pjsip as $setting) {
                    $pjsipSQL = "update pjsip set data = :data, flags = :flags where id = :id and keyword = :keyword;";
                    $stmt = $this->c->db->prepare($pjsipSQL);
                    foreach ($setting as $key => &$val) {
                        $stmt->bindParam($key, $val);
                    }
    ...
    public function deleteSIPTrunk($request, $response, $args){
        if (!isset($args['trunkName']) || $args['trunkName'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Trunk Name not defined.'
        ...
        $sql = "delete FROM trunks where trunkid = '". $trunkId ."'";
        $stmt = $this->c->db->query($sql);
        $result['trunk'] = $stmt->execute();

        $sql = "delete FROM trunk_dialpatterns where trunkid = '". $trunkId ."'";
        $stmt = $this->c->db->query($sql);
        $result['dialplan_patterns'] = $stmt->execute();

        $sql = "delete FROM trunk_dialpatterns where trunkid = '". $trunkId ."'";
        $stmt = $this->c->db->query($sql);
        $result['dialplan_patterns'] = $stmt->execute();

        $result['trunk_registerstring'] = $this->deleteTrunkRegisterString($trunkId);
        $result['trunk_usercontext'] = $this->deleteTrunkUserContext($trunkId);
        $result['trunk_peercontext'] = $this->deleteTrunkPeerContext($trunkId);
        $result['trunk_pjsip'] = $this->deleteTrunkPJSIP($trunkId);
    ...
    public function updateInboundRoute($request, $response, $args){
        global $destinationTypes;
        $body = $request->getParsedBody();

        if (!isset($args['routeName']) || $args['routeName'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Trunk Name not defined.'
            ));
        }
        elseif (!isset($args['did']) || $args['did'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'DID not defined.'
            ));
        }
        $routeDetails = $this->getInboundRouteDetails($args['routeName'], $args['did']);
        if(!$routeDetails){
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Invalid Route Name or DID.'
            ));
        }
        if(!isset($body['destination']) || $body['destination'] == ''){
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Destination not defined.'
            ));
        ...
        $body['did'] =  $args['did'];
        $body['description'] =  $args['routeName'];
        ...
        $incomingSQL = "update incoming set cidnum = :cidnum, destination = :destination, privacyman = :privacyman, alertinfo = :alertinfo, ringing = :ringing, fanswer = :fanswer, mohclass = :mohclass, grppre = :grppre, delay_answer = :delay_answer, pricid = :pricid, pmmaxretries = :pmmaxretries, pmminlength = :pmminlength, reversal = :reversal, rvolume = :rvolume, indication_zone = :indication_zone where description = :description and extension = :extension";
        $stmt = $this->c->db->prepare($incomingSQL);
        foreach ($incoming as $key => &$val) {
            $stmt->bindParam($key, $val);
        }
        ...
        $cidLookupSQL = "update cidlookup_incoming set cidlookup_id = :cidlookup_id, cidnum = :cidnum where extension = :extension";
        $stmt = $this->c->db->prepare($cidLookupSQL);
        foreach ($cidlookup as $key => &$val) {
            $stmt->bindParam($key, $val);
        }
        ...
        if(isset($body['fax_enabled']) && $body['fax_enabled'] == 1){
            ...
            $faxIncomingSQL = "update fax_incoming set cidnum = :cidnum, detection = :detection, detectionwait = :detectionwait, destination = :destination, legacy_email = :legacy_email, ring = :ring where extension = :extension";
            $stmt = $this->c->db->prepare($faxIncomingSQL);
            foreach ($faxIncoming as $key => &$val) {
                $stmt->bindParam($key, $val);
            }
        ...
        if(isset($body['superfecta_enabled']) && $body['superfecta_enabled'] == 1){
            ...
            $superfectaSQL = "update superfecta_to_incoming set cidnum = :cidnum, scheme = :scheme where extension = :extension";
            $stmt = $this->c->db->prepare($superfectaSQL);
            foreach ($superfecta as $key => &$val) {
                $stmt->bindParam($key, $val);
            }
        ...
        if($body['callrecording_enabled'] == 1){
            ...
            $callRecordingSQL = "update callrecording_module set cidnum = :cidnum, callrecording = :callrecording, display = :display where extension = :extension;";
            $stmt = $this->c->db->prepare($callRecordingSQL);
            foreach ($callRecording as $key => &$val) {
                $stmt->bindParam($key, $val);
            }
    ...
    public function deleteInboundRoute($request, $response, $args){
        if (!isset($args['routeName']) || $args['routeName'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Trunk Name not defined.'
        ...
        $sql = "delete FROM incoming where description = '". $routeName ."' and extension = '" . $did . "'";
        $stmt = $this->c->db->query($sql);
        $result['incoming'] = $stmt->execute();

        $sql = "delete FROM cidlookup_incoming where extension = '" . $did . "'";
        $stmt = $this->c->db->query($sql);
        $result['cidlookup_incoming'] = $stmt->execute();

        $sql = "delete FROM superfecta_to_incoming where extension = '" . $did . "'";
        $stmt = $this->c->db->query($sql);
        $result['cidlookup_incoming'] = $stmt->execute();

        $sql = "delete FROM fax_incoming where extension = '" . $did . "'";
        $stmt = $this->c->db->query($sql);
        $result['fax_incoming'] = $stmt->execute();

        $sql = "delete FROM callrecording_module where extension = '" . $did . "'";
        $stmt = $this->c->db->query($sql);
        $result['callrecording_module'] = $stmt->execute();
    ...
    public function createOutboundRoute($request, $response){
        ...
        if (isset($body['dialpatterns']) && count($body['dialpatterns']) > 0) {
            foreach ($body['dialpatterns'] as $dialPattern) {
                $duplicateDialPattern = $this->checkDuplicateOutboundDialPattern($dialPattern, $outboundRouteId);
        ...
        if($body['route']['callrecording_enabled'] == 1){
            $recordingValues = array('force', 'dontcare', 'yes', 'no', 'never');
            $callRecording = array(
    ...
    public function updateOutboundRoute($request, $response, $args){
        global $destinationTypes, $timezones;
        $body = $request->getParsedBody();
        ...
        $routeDetails = $this->getOutboundRouteDetails($args['routeName']);
        if(!$routeDetails){
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Invalid Route Name.'
            ));
        }

        $body['id'] =  $routeDetails['route_id'];
        $body['route']['name'] = (isset($body['route']['name'])) ? $body['route']['name'] : $args['routeName'];

        if((!isset($body['route']['emergency_route']) || $body['route']['emergency_route'] != '') && (!isset($body['route']['intracompany_route'])  || $body['route']['intracompany_route'] != '')){
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Emergency and Intra Company cannot be selected at the same time.'
            ));
        ...
        elseif(isset($body['dialpatterns']) && count($body['dialpatterns']) <= 0 ){
            return $response->withJson(array(
                'code' => 501,
                'data' => "Dial Patterns not defined."
            ));
        }
        elseif(isset($body['route']['failed_destination_type']) && !array_key_exists($body['route']['failed_destination_type'], $destinationTypes)){
            $output = implode(', ', array_map(
        ...
        $outboundRoutesSQL = "update outbound_routes set name = :name, outcid = :outcid, outcid_mode = :outcid_mode, password = :password, emergency_route = :emergency_route, intracompany_route = :intracompany_route, mohclass = :mohclass, time_group_id = :time_group_id, dest = :dest, time_mode = :time_mode, calendar_id = :calendar_id, calendar_group_id = :calendar_group_id, timezone = :timezone where route_id = :id";
        $stmt = $this->c->db->prepare($outboundRoutesSQL);
        foreach ($outboundRoutes as $key => &$val) {
            $stmt->bindParam($key, $val);
        }
        $outboundRoutesResult = $stmt->execute();
        $outboundRouteId = $body['id'];
            ...
            foreach ($body['dialpatterns'] as $dialPattern) {
                ...
                if(!$duplicateDialPattern){
                    ...
                    $pattern = array(
                        ...
                        'match_cid'                 =>  (isset($dialPattern['match_cid'])) ? $dialPattern['match_cid'] : "",
                        'prepend_digits'            =>  (isset($dialPattern['prepend_digits'])) ? $dialPattern['prepend_digits'] : 0,
                        'old_match_pattern_prefix'  =>  $duplicateDialPattern[0]['match_pattern_prefix'],
                        'old_match_pattern_pass'    =>  $duplicateDialPattern[0]['match_pattern_pass'],
                        'old_match_cid'             =>  $duplicateDialPattern[0]['match_cid'],
                        'old_prepend_digits'        =>  $duplicateDialPattern[0]['prepend_digits'],
                    );

                    $dialPatternSQL = "update outbound_route_patterns set match_pattern_prefix = :match_pattern_prefix, match_pattern_pass = :match_pattern_pass, match_cid = :match_cid, prepend_digits = :prepend_digits where route_id = :route_id and match_pattern_prefix = :old_match_pattern_prefix and match_pattern_pass = :old_match_pattern_pass and match_cid = :old_match_cid and prepend_digits = :old_prepend_digits";
                    $stmt = $this->c->db->prepare($dialPatternSQL);
                    foreach ($pattern as $key => &$val) {
                        $stmt->bindParam($key, $val);
                    }
        ...
        if (isset($body['trunks']) && count($body['trunks']) > 0) {
            ...
            if(isset($trunks) && count($trunks) > 0){
                foreach ($trunks as $setting) {
                    $duplicateTrunk = $this->checkDuplicateOutboundTrunk($outboundRouteId, $setting['trunk_id']);
                    ...
                    if(!$duplicateTrunk){
                        ...
                        $trunksSQL = "update outbound_route_trunks set trunk_id = :trunk_id, seq = :seq where route_id = :route_id and trunk_id = :trunk_id and seq = :seq";
                        $stmt = $this->c->db->prepare($trunksSQL);
                        foreach ($setting as $key => &$val) {
                            $stmt->bindParam($key, $val);
                        }
    ...
    public function deleteOutboundRoute($request, $response, $args){
        if (!isset($args['routeName']) || $args['routeName'] == '') {
            return $response->withJson(array(
                'code' => 501,
                'data' => 'Trunk Name not defined.'
        ...
        $sql = "delete FROM outbound_routes where route_id = '". $body['id'] ."'";
        $stmt = $this->c->db->query($sql);
        $result['outbound_routes'] = $stmt->execute();

        $sql = "delete FROM outbound_route_patterns where route_id = '" . $body['id'] . "'";
        $stmt = $this->c->db->query($sql);
        $result['outbound_route_patterns'] = $stmt->execute();

        $sql = "delete FROM outbound_route_trunks where route_id = '" . $body['id'] . "'";
        $stmt = $this->c->db->query($sql);
        $result['outbound_route_trunks'] = $stmt->execute();

        $sql = "delete FROM callrecording_module where extension = '" . $body['id'] . "'";
        $stmt = $this->c->db->query($sql);
        $result['callrecording_module'] = $stmt->execute();
